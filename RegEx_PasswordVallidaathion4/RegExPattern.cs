...
namespace RegEx_PasswordVallidaathion4
{
    class RegExPattern
    {
        ...
        string[] Inputs4 = { "Vish@lh1", "abc1sd&v", "$yzYzv3b", "Vishal1ath@d", "abcds", "#Abcds0x", "raj^8haI", "hvfcg5*" };
        string PassPattern4 = "^(?=.*[0-9])(?=.*[A-Z])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8}$";

        public void PAsswordValidation4()
        {
            Regex regex = new Regex(PassPattern4);
            Console.WriteLine(" Password Validation ");
            foreach (string word in Inputs4)
            {
                if (regex.IsMatch(word))
                {
                    Console.WriteLine(word + "---> valid");
